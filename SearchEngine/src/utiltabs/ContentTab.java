...
public class ContentTab extends UtilTab {
	...
	private Button search, trending;
	...
	public ContentTab(SearchTab searchTab, String id) throws NumberFormatException, Exception {
		...
		trending.setOnAction(e -> {
			goToTrending(null, null);
		});

		trending.setPrefSize(90, 40);
		trending.setTextAlignment(TextAlignment.CENTER);
		trending.setPadding(new Insets(-10, 0, -10, 0));
		
		
		HBox directBox = new HBox(5, backButton, forwardButton);
		
		HBox additionalHeaderBox = new HBox(10, search, trending);
		
		HBox headerBox = new HBox(200, directBox, additionalHeaderBox);
		...
		HBox.setHgrow(articleBox, Priority.NEVER);
		HBox.setHgrow(additionBox, Priority.NEVER); 
